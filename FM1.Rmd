...
author: "Leydi Laura Bravo Castro"
date: "21/10/2024"
output:
...
s1 <- cos(10 * pi * t)
s2 <- cos(20 * pi * t)
s3 <- (4 / pi) * sin(6 * pi * t) + (1 / 3) * sin(18 * pi * t)
fc1 <- cos(2 * pi * 30 * t)  # Portadora 1
fc2 <- cos(2 * pi * 50 * t)  # Portadora 2
fc3 <- cos(2 * pi * 70 * t)  # Portadora 3
mod_s1 <- s1 * fc1
mod_s2 <- s2 * fc2
mod_s3 <- s3 * fc3
multiplexed_signal <- mod_s1 + mod_s2 + mod_s3
highchart() %>% 
  hc_add_series(cbind(t, mod_s1), name = "Modulada S1") %>% 
  hc_add_series(cbind(t, mod_s2), name = "Modulada S2") %>% 
  hc_add_series(cbind(t, mod_s3), name = "Modulada S3") %>% 
  hc_add_series(cbind(t, multiplexed_signal), name = "Señal Multiplexada", color = "#ffb703") %>%
  hc_title(text = "Multiplexación FDM ", useHTML = TRUE) %>% 
